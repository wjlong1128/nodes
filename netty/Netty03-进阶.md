...
>
>   ![](https://picgo-images0401.oss-cn-beijing.aliyuncs.com/picgo/0049.png)
>
> 
>
> * 为了解决此问题，引入了窗口概念，窗口大小即决定了无需等待应答而可以继续发送的数据最大值
>
>   ![](https://picgo-images0401.oss-cn-beijing.aliyuncs.com/picgo/0051.png)
>
...
```java
// 最大长度，长度偏移，长度占用字节，长度调整（从长度字节跳过之后在移动几位之后的才是内容），剥离字节数
...
```java
参数1: maxFrameLength 内容占用的最大长度
参数2: lengthFieldOffset 长度偏移量 解读从那儿开始
参数3: lengthFieldLength 表示真正内容占用多少字节的字节的长度
参数4: lengthAdjustment 长度字节之后跳过多少个字节的才是内容
参数5: initialBytesToStrip 解析之后 从头开始跳过几个字节
```
...
![](https://picgo-images0401.oss-cn-beijing.aliyuncs.com/picgo/0013.png)
...
> 
> 对于LengthFieldBasedFrameDecoder 这样的handler来说 每一次处理都会记录状态 如果抽取成局部变量 或者常量的话 EventLoopGruop里面的不同线程h
> ```
